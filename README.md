...
- ⚙️ Запуск кода на ядре Linux с минимальной настройкой (в простейшем виде только путь к папке с бинарями и путь к ядру Linux)
...
| -------------- | ------------------------------------------------- |
| `console-unix` | Привязка консоли QEMU к UNIX-сокету хоста         |
